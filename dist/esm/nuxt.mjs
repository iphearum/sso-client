...
// src/nuxt.ts
import { onMounted, useState } from "nuxt/app";
...
function useSession(options) {
  ...
  const client = providedClient ?? getCachedClient("nuxt", void 0, config, (cfg) => new AuthClient(cfg));
  const data = useState("smis:sso:session", () => null);
  const status = useState("smis:sso:status", () => "loading");
