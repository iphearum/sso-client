...
var readEnvString = (...keys) => readEnvStringFrom(void 0, ...keys);
...
var readEnvNumber = (...keys) => readEnvNumberFrom(void 0, ...keys);

// src/runtime-bootstrap.ts
var readNextRuntimeEnv = () => ({
  SMIS_APP_KEY: typeof process !== "undefined" ? process.env.SMIS_APP_KEY : void 0,
  NEXT_PUBLIC_SMIS_APP_KEY: typeof process !== "undefined" ? process.env.NEXT_PUBLIC_SMIS_APP_KEY : void 0,
  NEXTAUTH_SMIS_APP_KEY: typeof process !== "undefined" ? process.env.NEXTAUTH_SMIS_APP_KEY : void 0,
  SMIS_AUTH_BASE_URL: typeof process !== "undefined" ? process.env.SMIS_AUTH_BASE_URL : void 0,
  NEXT_PUBLIC_SMIS_AUTH_BASE_URL: typeof process !== "undefined" ? process.env.NEXT_PUBLIC_SMIS_AUTH_BASE_URL : void 0,
  SMIS_PROBE_PATH: typeof process !== "undefined" ? process.env.SMIS_PROBE_PATH : void 0,
  NEXT_PUBLIC_SMIS_PROBE_PATH: typeof process !== "undefined" ? process.env.NEXT_PUBLIC_SMIS_PROBE_PATH : void 0,
  SMIS_STORAGE: typeof process !== "undefined" ? process.env.SMIS_STORAGE : void 0,
  NEXT_PUBLIC_SMIS_STORAGE: typeof process !== "undefined" ? process.env.NEXT_PUBLIC_SMIS_STORAGE : void 0,
  SMIS_STORAGE_KEY: typeof process !== "undefined" ? process.env.SMIS_STORAGE_KEY : void 0,
  NEXT_PUBLIC_SMIS_STORAGE_KEY: typeof process !== "undefined" ? process.env.NEXT_PUBLIC_SMIS_STORAGE_KEY : void 0,
  SMIS_TIMEOUT_MS: typeof process !== "undefined" ? process.env.SMIS_TIMEOUT_MS : void 0,
  NEXT_PUBLIC_SMIS_TIMEOUT_MS: typeof process !== "undefined" ? process.env.NEXT_PUBLIC_SMIS_TIMEOUT_MS : void 0,
  SMIS_POLL_INTERVAL_MS: typeof process !== "undefined" ? process.env.SMIS_POLL_INTERVAL_MS : void 0,
  NEXT_PUBLIC_SMIS_POLL_INTERVAL_MS: typeof process !== "undefined" ? process.env.NEXT_PUBLIC_SMIS_POLL_INTERVAL_MS : void 0
});
var bootstrapRuntimeEnv = () => {
  if (typeof process === "undefined") return;
  const inferred = readNextRuntimeEnv();
  for (const value of Object.values(inferred)) {
    if (typeof value === "string" && value.length > 0) {
      setRuntimeEnv(inferred);
      return;
    }
  }
};
...
import { signIn as nextSignIn, signOut as nextSignOut, useSession as nextUseSession } from "next-auth/react";
...
// src/next-auth.ts
var NextAuthClient = class extends AuthClient {
  async ensureNextAuthSession(adapter, options) {
    const { sessionHook, signIn: signIn2 } = adapter;
    const providerId = options?.providerId ?? "credentials";
    const redirect = options?.redirect ?? false;
    const session = options?.session ?? await this.ensureSession();
    const alreadySynced = sessionHook.status === "authenticated" && Boolean(sessionHook.data?.sso?.accessToken === session.accessToken);
    if (alreadySynced || sessionHook.status === "loading") {
      return session;
    }
    await signIn2(providerId, {
...
var getClient = (client, config) => getCachedClient("next-auth", client, config, (cfg) => new NextAuthClient(resolveConfig(cfg)));
...
async function signOut(options) {
  ...
  await getClient(client, config).signOut().catch(() => void 0);
  await nextSignOut(nextAuthOptions);
}
function useSession(options) {
  ...
  try {
    nextAuthSessionHook = nextUseSession();
  } catch (error) {
  ...
  useEffect(() => {
    if (providerMissingDetected && !missingProvider) {
      console.warn(
        "SMIS SSO: NextAuth SessionProvider is missing; falling back to local session state."
      );
  ...
  const ensureKey = useMemo(() => JSON.stringify(ensureOptions ?? {}), [ensureOptions]);
  ...
  const memoizedEnsureOptions = useMemo(() => ensureOptions, [ensureKey]);
  useEffect(() => {
    if (!client) return;
    if (sessionHook.status === "loading") return;
    void client.ensureNextAuthSession({ sessionHook, signIn: nextSignIn }, memoizedEnsureOptions).then((resolvedSession) => {
      if (missingProvider) {
        setFallbackSession({
