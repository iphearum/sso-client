...
const inferStorage = (config) => {
    const env = (0, env_1.readEnvStringFrom)(config?.env, "SMIS_STORAGE", "NEXT_PUBLIC_SMIS_STORAGE");
    const preferred = config?.storage ?? env;
    if (preferred === "sessionStorage" || preferred === "memory")
        return preferred;
    return "localStorage";
};
const inferStorageKey = (config, appKey) => config?.storageKey ??
    (0, env_1.readEnvStringFrom)(config?.env, "SMIS_STORAGE_KEY", "NEXT_PUBLIC_SMIS_STORAGE_KEY") ??
    `smis-sso:${appKey}`;
const inferTimeout = (config) => config?.timeoutMs ?? (0, env_1.readEnvNumberFrom)(config?.env, "SMIS_TIMEOUT_MS", "NEXT_PUBLIC_SMIS_TIMEOUT_MS");
const inferPollInterval = (config) => config?.pollIntervalMs ?? (0, env_1.readEnvNumberFrom)(config?.env, "SMIS_POLL_INTERVAL_MS", "NEXT_PUBLIC_SMIS_POLL_INTERVAL_MS");
